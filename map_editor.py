...
class MapEditor:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    event.key == pygame.K_s and pygame.key.get_pressed()[pygame.K_LCTRL]
                ):
                    self.save_map()
                elif (
                    event.key == pygame.K_o and pygame.key.get_pressed()[pygame.K_LCTRL]
                ):
                    self.load_map()
                elif (
                    event.key == pygame.K_n and pygame.key.get_pressed()[pygame.K_LCTRL]
                ):
                    self.new_map()
    ...
    def is_town_hall_tile(self, tile_x: int, tile_y: int) -> bool:
        ...
        return (
            center_x - 1 <= tile_x <= center_x + 1
            and center_y - 1 <= tile_y <= center_y + 1
        )
    ...
    def handle_left_click(self, pos: Tuple[int, int]):
        ...
        if (
            0 <= tile_x < self.map_width
            and 0 <= tile_y < self.map_height
            and self.last_drawn_tile != (tile_x, tile_y)
            and not self.is_town_hall_tile(tile_x, tile_y)
        ):
    ...
    def render_map(self):
        ...
        end_x = min(
            self.map_width, (self.camera_x + self.screen_width - 220) // TILE_SIZE + 1
        )
        end_y = min(
            self.map_height, (self.camera_y + self.screen_height) // TILE_SIZE + 1
        )
        ...
        for y in range(start_y, end_y):
            for x in range(start_x, end_x):
                ...
                # Render tile
                pygame.draw.rect(
                    self.screen, color, (screen_x, screen_y, TILE_SIZE, TILE_SIZE)
                )
    ...
    def render_grid(self):
        ...
        start_y = max(0, self.camera_y // TILE_SIZE)
        end_x = min(
            self.map_width, (self.camera_x + self.screen_width - 220) // TILE_SIZE + 2
        )
        end_y = min(
            self.map_height, (self.camera_y + self.screen_height) // TILE_SIZE + 2
        )
    ...
    def render_palette(self):
        ...
        for i, tile_type in enumerate(self.tile_types):
            ...
            self.screen.blit(
                label, (self.palette_x + self.palette_tile_size + 5, y + 10)
            )
            ...
            self.screen.blit(
                hotkey_text, (self.palette_x + self.palette_tile_size + 5, y + 25)
            )

    def render_info(self):
        """Render information panel"""
        info_y = (
            self.palette_y + len(self.tile_types) * (self.palette_tile_size + 5) + 40
        )
    ...
    def load_map(self):
        ...
        try:
            ...
            if 1 <= choice_num <= len(map_files):
                ...
                print(
                    f"❌ Invalid choice. Please enter 1-{len(map_files)} or 0 to cancel"
                )
                return
        ...
        try:
            ...
            if "map_data" not in map_data:
                print("❌ Invalid map file: missing map_data")
                return
    ...
    def new_map(self):
        ...
        print(
            f"🗺️ Created new map with 3x3 town hall centered at ({center_x}, {center_y})"
        )
    ...
    def run(self):
        ...
        print(
            "   - 1-8: Select tile type (1=Empty, 2=Wood, 3=Stone, 4=Wheat, 5=Metal, 6=Gold, 7=Wall, 8=Town Hall)"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--width", type=int, default=80, help="Map width in tiles (default: 80)"
    )
    parser.add_argument(
        "--height", type=int, default=60, help="Map height in tiles (default: 60)"
    )
