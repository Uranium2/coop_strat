...
async def test_building_placement_issue():
    ...
    while iteration < max_iterations:
        ...
        if iteration % 10 == 0:
            print(
                f"   Iteration {iteration}: Hero at ({hero.position.x:.2f}, {hero.position.y:.2f})"
            )
    ...
    print(
        f"Building covers tiles: ({building_tile_x}, {building_tile_y}) to ({building_tile_x + size[0] - 1}, {building_tile_y + size[1] - 1})"
    )
    ...
    build_success = game_manager.build_structure(
        player_id, BuildingType.BARRACKS, building_position
    )
    ...
    if not is_adjacent:
        ...
        for test_pos in [
            ...
            # Test adjacency
            test_distance = math.sqrt(
                (test_pos[0] - building_tile_x) ** 2
                + (test_pos[1] - building_tile_y) ** 2
            )
            ...
            print(
                f"   Hero at ({test_pos[0]}, {test_pos[1]}): distance={test_distance:.2f}, adjacent={test_adjacent}"
            )

            if test_adjacent:
                print(
                    f"   ✅ Hero is adjacent at position ({test_pos[0]}, {test_pos[1]})"
                )
                break
