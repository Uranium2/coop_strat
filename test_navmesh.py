...
from server.services.navmesh import NavMesh, NavMeshPathfinder
...
def test_navmesh_pathfinding():
    ...
    try:
        ...
        for poly_id, polygon in pathfinder.navmesh.polygons.items():
            print(
                f"  Polygon {poly_id}: center at ({polygon.center.x:.1f}, {polygon.center.y:.1f})"
            )
...
def test_polygon_generation():
    ...
    print(
        f"Generated {len(navmesh.polygons)} polygons for simple 5x5 map with 1 obstacle"
    )
