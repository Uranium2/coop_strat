...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, player_id: str):
        ...
        logger.info(f"Player {player_id} connected successfully. Total connections: {len(self.active_connections)}")
    ...
    async def send_to_player(self, player_id: str, message: dict):
        if player_id in self.active_connections:
            try:
                logger.debug(f"Sending message to {player_id}: {message.get('type', 'unknown')}")
    ...
    async def send_to_lobby(self, lobby_id: str, message: dict):
        logger.debug(f"Sending message to lobby {lobby_id}: {message.get('type', 'unknown')}")
        ...
        if lobby:
            ...
            logger.warning(f"Attempted to send message to non-existent lobby {lobby_id}")
...
async def handle_message(player_id: str, message: dict):
    ...
    if message_type == "create_lobby":
        ...
        if success:
            ...
            logger.info(f"Lobby {lobby_id} created successfully with player {player_id}")
        ...
        if current_lobby:
            logger.info(f"Removing player {player_id} from previous lobby {current_lobby}")
            lobby_manager.remove_player(current_lobby, player_id)
            await manager.send_to_lobby(current_lobby, {
                "type": "player_disconnected",
                "player_id": player_id
            })
        ...
        if lobby_id:
            ...
            if lobby and lobby.can_start_game():
                ...
                try:
                    ...
                    logger.info(f"Game state created successfully, serializing...")
                    game_state_dict = initial_state.dict()
                    logger.info(f"Game state serialized successfully")
                    ...
                    logger.error(f"Failed to start game for lobby {lobby_id}: {e}", exc_info=True)
            else:
                logger.warning(f"Cannot start game for lobby {lobby_id} - conditions not met")
        else:
            logger.warning(f"Player {player_id} not in any lobby when trying to start game")
    
    elif message_type == "game_action":
        lobby_id = manager.player_lobbies.get(player_id)
        if lobby_id:
            action = message.get("action")
            logger.debug(f"Game action from {player_id} in lobby {lobby_id}: {action.get('type', 'unknown')}")
...
async def handle_game_action(lobby_id: str, player_id: str, action: dict):
    ...
    if action_type == "move_hero":
        ...
        from shared.models.game_models import Position, BuildingType
        position = Position(x=position_data["x"], y=position_data["y"])
        
        if game_manager.build_structure(player_id, BuildingType(building_type), position):
            logger.debug(f"Building {building_type} placed successfully for {player_id}")
            updated_state = game_manager.get_game_state()
            await manager.send_to_lobby(lobby_id, {
                "type": "game_update",
                "game_state": updated_state.dict()
