...
class NavMesh:
    ...
    def generate_from_tile_map(
        self, tile_map: List[List[TileType]], buildings: Optional[Dict] = None
    ):
    ...
    def _create_polygon_from_rectangle(self, rect: Tuple[int, int, int, int]):
        ...
        # Calculate center
        center = Position(x=float(x + width / 2), y=float(y + height / 2))
    ...
    def _find_shared_edge(
        self, poly1: NavMeshPolygon, poly2: NavMeshPolygon
    ...
    def _edges_overlap(
        self, e1_start: Position, e1_end: Position, e2_start: Position, e2_end: Position
    ) -> bool:
    ...
    def find_path_polygons(
        self, start_point: Position, goal_point: Position
    ) -> List[int]:
        ...
        while open_set:
            ...
            for neighbor_id in current_polygon.neighbors:
                ...
                if current.entry_point:
                    move_cost = self._distance(
                        current.entry_point, neighbor_entry_point
                    )
                else:
...
class NavMeshPathfinder:
    ...
    def _funnel_algorithm(
        self, start: Position, goal: Position, polygon_path: List[int]
    ) -> List[Position]:
