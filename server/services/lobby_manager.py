        ...
        if self.redis_client:
            try:
                ...
                self.redis_client.setex(
                    key, 3600, json.dumps(lobby_data)
                )  # 1 hour expiry
...
class Lobby:
    ...
    def add_player(self, player_id: str, player_name: str) -> bool:
        if self.is_game_active:
            logger.warning(
                f"Cannot add player {player_id} to lobby {self.lobby_id} - game is active"
            )
            return False

        if len(self.players) >= self.max_players:
            logger.warning(
                f"Cannot add player {player_id} to lobby {self.lobby_id} - lobby full"
            )
            return False
        ...
        self.last_activity = time.time()
        logger.info(
            f"Added player {player_id} ({player_name}) to lobby {self.lobby_id} - now {len(self.players)}/{self.max_players} players"
        )
        self.save()
        return True
    ...
    def can_start_game(self) -> bool:
        ...
        players_with_heroes = sum(
            1 for player in self.players.values() if player.hero_type
        )
        can_start = (
            len(self.players) >= 1
            and not self.is_game_active
            and players_with_heroes >= 1
        )
        logger.info(
            f"Lobby {self.lobby_id} can start game: {can_start} (players: {len(self.players)}, active: {self.is_game_active}, heroes: {players_with_heroes})"
        )
        return can_start
...
class LobbyManager:
    ...
    def create_lobby(self, lobby_id: str) -> Lobby:
        ...
        lobby.save()
        logger.info(
            f"Lobby {lobby_id} created successfully. Total lobbies: {len(self.lobbies)}"
        )
        return lobby
        ...
        if lobby:
            ...
            if len(lobby.players) == 0:
                if lobby.is_game_active:
                    ...
                    logger.info(
                        f"Lobby {lobby_id} is empty after game was active, resetting to waiting state"
                    )
                    ...
                    logger.info(
                        f"Lobby {lobby_id} is empty (waiting state), keeping for potential rejoin"
                    )
                    lobby.save()
    ...
    def list_active_lobbies(self) -> List[dict]:
        ...
        logger.info(
            f"Total lobbies to check: {len(all_lobby_ids)} (DB: {len(self.db_manager.list_lobbies())}, Memory: {len(self.lobbies)})"
        )
        ...
        for lobby_id in all_lobby_ids:
            ...
            if lobby:
                ...
                logger.info(
                    f"Processing lobby {lobby_id}: players={len(lobby.players)}, is_game_active={lobby.is_game_active}, age={age_minutes:.1f}min"
                )

                # Clean up very old empty lobbies (older than 15 minutes)
                if (
                    len(lobby.players) == 0
                    and (current_time - lobby.last_activity) > 900
                ):  # 15 minutes
                    logger.info(
                        f"Cleaning up old empty lobby {lobby_id} (age: {age_minutes:.1f} minutes)"
                    )
                ...
                logger.warning(
                    f"Could not load lobby {lobby_id} from database or memory"
                )

        logger.info(
            f"Final result: Returning {len(lobbies)} lobbies: {[lobby['lobby_id'][:8] + '...' for lobby in lobbies]}"
        )
        return lobbies
