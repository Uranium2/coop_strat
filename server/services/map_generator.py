...
from shared.models.game_models import TileType, MapTile
from shared.constants.game_constants import MAP_WIDTH, MAP_HEIGHT

class MapGenerator:
    def __init__(self, width: int = MAP_WIDTH, height: int = MAP_HEIGHT, seed: int = None):
        self.width = width
        self.height = height
        if seed:
            random.seed(seed)
    ...
    def generate_map(self) -> List[List[TileType]]:
        map_data = [[TileType.EMPTY for _ in range(self.width)] for _ in range(self.height)]
    ...
    def _place_basic_resources(self, map_data: List[List[TileType]], center_x: int, center_y: int, clear_radius: int):
        ...
        for resource in basic_resources:
            ...
            for _ in range(clusters):
                ...
                if self._distance(cluster_x, cluster_y, center_x, center_y) < clear_radius + 10:
                    continue
                ...
                for _ in range(cluster_size):
                    ...
                    if (0 <= x < self.width and 0 <= y < self.height and 
                        map_data[y][x] == TileType.EMPTY and
                        self._distance(x, y, center_x, center_y) >= clear_radius):
                        map_data[y][x] = resource
    
    def _place_rare_resources(self, map_data: List[List[TileType]], center_x: int, center_y: int):
        ...
        for resource in rare_resources:
            ...
            for _ in range(clusters):
                ...
                for _ in range(cluster_size):
                    ...
                    if (0 <= x < self.width and 0 <= y < self.height and 
                        map_data[y][x] == TileType.EMPTY):
                        map_data[y][x] = resource
    
    def _ensure_symmetry(self, map_data: List[List[TileType]], center_x: int, center_y: int):
        for y in range(self.height):
            for x in range(self.width):
                if map_data[y][x] != TileType.EMPTY:
                    ...
                    if (0 <= mirror_x < self.width and 0 <= mirror_y < self.height and
                        map_data[mirror_y][mirror_x] == TileType.EMPTY):
    ...
    def get_spawn_area(self) -> Tuple[int, int, int, int]:
        center_x, center_y = self.width // 2, self.height // 2
        spawn_radius = 10
        return (center_x - spawn_radius, center_y - spawn_radius, 
                center_x + spawn_radius, center_y + spawn_radius)