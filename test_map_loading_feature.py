...
def test_map_loading_feature():
    ...
    if loaded_map:
        ...
        print(
            f"  ✅ Successfully loaded: {len(map_data)}x{len(map_data[0]) if map_data else 0}"
        )

        # Count town halls in loaded map
        town_halls = sum(
            1 for row in map_data for tile in row if tile.value == "TOWN_HALL"
        )
    ...
    try:
        ...
        print("  ✅ Game created with custom map!")
        ...
        town_halls = [
            ...
            if b.building_type.value == "TOWN_HALL"
        ]
        if town_halls:
            th = town_halls[0]
    ...
    try:
        ...
        print("  ✅ Game created with generated map!")
        print(
            f"  🗺️ Map size: {len(game_state2.map_data)}x{len(game_state2.map_data[0])}"
        )
    ...
    try:
        game_manager3 = GameManager(
            "test_lobby3", players, custom_map="nonexistent.json"
        )
        game_state3 = game_manager3.get_game_state()

        print("  ✅ Game created with fallback to generated map!")
        print(
            f"  🗺️ Map size: {len(game_state3.map_data)}x{len(game_state3.map_data[0])}"
        )
