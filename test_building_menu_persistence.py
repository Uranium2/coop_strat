...
async def test_building_menu_persistence():
    ...
    network_manager.send_game_action = Mock(
    ...
    left_click_event = pygame.event.Event(
        pygame.MOUSEBUTTONDOWN,
        {
            "button": 1,
            "pos": (
                hero_screen_pos[0] + 16,
                hero_screen_pos[1] + 16,
            ),  # Center of hero tile
        },
    )
    ...
    assert game_scene.building_menu.visible, (
        "Building menu should be visible when hero is selected"
    )
    ...
    assert game_scene.building_menu.visible, (
        "Building menu should still be visible during placement preview"
    )
    ...
    right_click_event = pygame.event.Event(
        pygame.MOUSEBUTTONDOWN,
        {
            "button": 3,
    ...
    assert not game_scene.building_placer.is_placing(), (
        "Building placer should be inactive after right-click"
    )
    assert game_scene.selected_entity is not None, "Hero should still be selected"
    assert game_scene.building_menu.visible, (
        "Building menu should still be visible after cancelling placement"
    )
    ...
    placement_click_event = pygame.event.Event(
        pygame.MOUSEBUTTONDOWN,
        {
            "button": 1,
    ...
    assert not game_scene.building_placer.is_placing(), (
        "Building placer should be inactive after placement"
    )
    assert game_scene.selected_entity is not None, "Hero should still be selected"
    assert game_scene.building_menu.visible, (
        "Building menu should still be visible after placing building"
    )
    ...
    deselect_click_event = pygame.event.Event(
        pygame.MOUSEBUTTONDOWN,
        {
            "button": 3,
    ...
    assert not game_scene.building_menu.visible, (
        "Building menu should be hidden when hero is deselected"
    )
