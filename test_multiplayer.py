...
class TestPlayer:
    ...
    def _on_game_started(self, data):
        ...
        print(
            f"{self.name}: Map size: {len(self.game_state['map_data'])}x{len(self.game_state['map_data'][0])}"
        )
        print(f"{self.name}: Heroes: {list(self.game_state['heroes'].keys())}")
        print(
            f"{self.name}: Lobby ID in game state: {self.game_state.get('lobby_id', 'NOT FOUND')}"
        )
    ...
    try:
        ...
        if heroes1 == heroes2:
            ...
            for hero_id, hero in heroes1.items():
                ...
                print(
                    f"  Hero {hero_id[:8]}: Player {player_id[:8]} ({hero_type}) at ({position['x']}, {position['y']})"
                )
        else:
