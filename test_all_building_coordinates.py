...
from shared.constants.game_constants import MAP_HEIGHT, MAP_WIDTH, TILE_SIZE
...
def test_all_building_coordinates():
    ...
    print(
        f"   Town Hall: pixels ({town_hall_pixel_x}, {town_hall_pixel_y}) -> tiles ({town_hall_render_tile_x}, {town_hall_render_tile_y})"
    )
    print(
        f"   Farm: pixels ({farm_pixel_x}, {farm_pixel_y}) -> tiles ({farm_render_tile_x}, {farm_render_tile_y})"
    )

    # With camera at origin, screen positions should be:
    screen_x_townhall = (
        town_hall_render_tile_x * TILE_SIZE
    )  # Should equal town_hall_pixel_x
    screen_y_townhall = (
        town_hall_render_tile_y * TILE_SIZE
    )  # Should equal town_hall_pixel_y
    ...
    print("\n✅ VERIFICATION:")
    print(
        f"   Town Hall: pixel->tile->screen = {town_hall_pixel_x} -> {town_hall_render_tile_x} -> {screen_x_townhall} ✓"
    )
    print(
        f"   Farm: pixel->tile->screen = {farm_pixel_x} -> {farm_render_tile_x} -> {screen_x_farm} ✓"
    )
    print("   Both buildings now use PIXEL coordinates and convert consistently!")
