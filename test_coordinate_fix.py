...
async def test_coordinate_conversion():
    ...
    if success:
        ...
        if game_manager.game_state.buildings:
            ...
            print(
                f"🏢 Building created at: ({building.position.x}, {building.position.y})"
            )
            ...
            if expected_tile_x == build_tile_x and expected_tile_y == build_tile_y:
                ...
                print(
                    f"❌ Building placed at wrong tile! Expected ({build_tile_x}, {build_tile_y})"
                )
        else:
        ...
        if building_info:
            is_valid = game_manager._is_valid_build_position(
                build_position, building_info["size"]
            )
            print(f"📏 Position valid: {is_valid}")

            # Check adjacency
            is_adjacent = game_manager._is_hero_adjacent_to_build_location(
                player_id, build_position, building_info["size"]
            )
