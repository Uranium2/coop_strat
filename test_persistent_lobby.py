...
async def test_persistent_lobby():
    ...
    print(f"1️⃣ Client 1 connecting and creating lobby...")
                ...
                try:
                    async with websockets.connect(client2_uri) as websocket2:
                        ...
                        if data2.get("type") == "player_joined":
                            print(f"[Client 2] ✅ Successfully joined lobby!")
                            print(f"[Client 2] Players in lobby: {data2.get('players', [])}")
                            ...
                            try:
                                ...
                                if data1.get("type") == "player_joined":
                                    print(f"[Client 1] ✅ Notified of new player!")
                                    print(f"🎉 SUCCESS: Both clients can join the same lobby!")
                                else:
                                    print(f"[Client 1] ❌ Unexpected message: {data1}")
                            except asyncio.TimeoutError:
                                print(f"[Client 1] ❌ Did not receive player_joined notification")
                                
                        elif data2.get("type") == "join_failed":
                            print(f"[Client 2] ❌ Join failed: {data2.get('reason', 'Unknown')}")
                            return False
                        else:
