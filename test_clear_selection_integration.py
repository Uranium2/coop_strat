...
async def test_clear_selection_integration():
    ...
    game_scene.building_menu.clear_selection = mock_clear_selection(
        original_clear_selection
    )
    ...
    print(
        "   Calling building_placer.stop_placement() and building_menu.clear_selection()..."
    )
    ...
    print(
        f"   Building placer active after stop: {game_scene.building_placer.is_placing()}"
    )
    print(
        f"   Building selected after clear: {game_scene.building_menu.selected_building_type}"
    )

    print(f"\n🎉 Test completed! clear_selection() was called {call_count} time(s)")
    print(
        "✅ The code change successfully calls clear_selection() after building placement!"
    )
