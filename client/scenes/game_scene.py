...
class GameScene:
    def __init__(self, screen: pygame.Surface, network_manager: NetworkManager, initial_game_state: Dict[str, Any]):
        ...
        self.network_manager.register_handler("building_placed", self._on_building_placed)
    ...
    def _handle_build_click(self, pos: tuple):
        ...
        if self.build_mode and self.selected_entity and hasattr(self.selected_entity, 'hero_type'):
    ...
    def _handle_left_click(self, pos: tuple):
        ...
        if hero:
            distance = ((hero.position.x - world_pos[0])**2 + (hero.position.y - world_pos[1])**2)**0.5
            if distance <= 2:
                self.selected_entity = hero
                return
        
        for building in self.game_state.buildings.values():
            if (building.position.x <= world_pos[0] < building.position.x + building.size[0] and
                building.position.y <= world_pos[1] < building.position.y + building.size[1]):
    ...
    def _handle_camera_movement(self, key):
        ...
        if key == pygame.K_UP:
            ...
            self.camera_y = min(MAP_HEIGHT * TILE_SIZE - self.screen.get_height(), self.camera_y + speed)
        elif key == pygame.K_LEFT:
            self.camera_x = max(0, self.camera_x - speed)
        elif key == pygame.K_RIGHT:
            self.camera_x = min(MAP_WIDTH * TILE_SIZE - self.screen.get_width(), self.camera_x + speed)
    ...
    def _handle_mouse_edge_scrolling(self):
        ...
        if mouse_x < edge_margin:
            ...
            self.camera_x = min(MAP_WIDTH * TILE_SIZE - screen_width, self.camera_x + scroll_speed)
        ...
        if mouse_y < edge_margin:
            ...
            self.camera_y = min(MAP_HEIGHT * TILE_SIZE - screen_height, self.camera_y + scroll_speed)
    ...
    def render(self, screen: pygame.Surface):
        ...
        self.minimap.render(screen, self.game_state, self.network_manager.player_id, 
                           self.camera_x, self.camera_y, self.screen.get_width(), self.screen.get_height())
    ...
    def _render_lobby_info(self, screen: pygame.Surface):
        ...
        if lobby_id:
            ...
            text = f"Lobby: {lobby_id[:12]}..." if len(lobby_id) > 15 else f"Lobby: {lobby_id}"
    ...
    def _render_map(self, screen: pygame.Surface):
        ...
        end_x = min(MAP_WIDTH, (int(self.camera_x) + screen.get_width()) // TILE_SIZE + 1)
        end_y = min(MAP_HEIGHT, (int(self.camera_y) + screen.get_height()) // TILE_SIZE + 1)
        
        for y in range(start_y, end_y):
            for x in range(start_x, end_x):
                if 0 <= y < len(self.game_state.map_data) and 0 <= x < len(self.game_state.map_data[y]):
    ...
    def _render_buildings(self, screen: pygame.Surface):
        for building in self.game_state.buildings.values():
            screen_pos = self._world_to_screen((building.position.x, building.position.y))
            
            if (-TILE_SIZE < screen_pos[0] < screen.get_width() and
                -TILE_SIZE < screen_pos[1] < screen.get_height()):
    ...
    def _render_heroes(self, screen: pygame.Surface):
        ...
        for hero in self.game_state.heroes.values():
            ...
            if (-TILE_SIZE < screen_pos[0] < screen.get_width() and
                ...
                pygame.draw.circle(screen, COLORS["WHITE"], (center_x, center_y), TILE_SIZE // 3, 2)
                
                # Render player name above hero
                player = self.game_state.players.get(hero.player_id)
                if player:
                    player_name = player.name if hasattr(player, 'name') else hero.player_id[:8]
    ...
    def _render_enemies(self, screen: pygame.Surface):
        for enemy in self.game_state.enemies.values():
            ...
            if (-TILE_SIZE < screen_pos[0] < screen.get_width() and
                ...
                center_x = screen_pos[0] + TILE_SIZE // 2
                center_y = screen_pos[1] + TILE_SIZE // 2
                pygame.draw.circle(screen, COLORS["RED"], (center_x, center_y), TILE_SIZE // 4)
                pygame.draw.circle(screen, COLORS["WHITE"], (center_x, center_y), TILE_SIZE // 4, 1)
    
    def _render_fog_of_war(self, screen: pygame.Surface):
        fog_rect = pygame.Rect(-int(self.camera_x), -int(self.camera_y), 
