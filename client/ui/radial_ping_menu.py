...
class RadialPingMenu:
    ...
    def draw(self, screen: pygame.Surface):
        ...
        menu_surface = pygame.Surface(
            (self.radius * 2 + 40, self.radius * 2 + 40), pygame.SRCALPHA
        )
        ...
        if self.hovered_ping:
            ...
            if hovered_color:
                ...
                menu_center = (
                    ...
                    menu_surface.get_height() // 2,
                )

                # Create a larger transparent surface for the sector
                sector_surface = pygame.Surface(
                    (self.radius * 2 + 40, self.radius * 2 + 40), pygame.SRCALPHA
                )

                # Draw the sector highlight (simplified - just draw lines)
                pygame.draw.circle(
                    sector_surface, (*hovered_color, 60), menu_center, self.radius
                )
        ...
        for ping_type, angle, label, color in self.ping_options:
            ...
            pygame.draw.circle(
                screen, (255, 255, 255), (int(sector_x), int(sector_y)), 15, 2
            )
        ...
        instruction_rect = instruction_surface.get_rect(
            center=(center_x, center_y - self.radius - 30)
        )
