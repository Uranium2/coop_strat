...
class BuildingMenu:
            ...
            for building_type in building_types:
                if building_type in [bt.value for bt in BuildingType]:
                    ...
                    x = (
                        self.menu_x
                        + 20
                        + col * (self.button_width + self.button_margin)
                    )
                    y = (
                        self.menu_y
                        + 50
                        + row * (self.button_height + self.button_margin)
                    )
    ...
    def handle_event(self, event: pygame.event.Event) -> Optional[BuildingType]:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:  # Left click
                mouse_pos = (
                    event.pos
                )  # Use event position instead of pygame.mouse.get_pos()
    ...
    def render(self, screen: pygame.Surface):
        ...
        close_text = self.small_font.render(
            "Press ESC or click outside to close", True, COLORS["GRAY"]
        )
        screen.blit(close_text, (self.menu_x + 20, self.menu_y + self.menu_height - 30))

    def _render_building_button(
        self, screen: pygame.Surface, building_type: str, button_info: Dict
    ):
        ...
        if can_afford:
            if (
                ...
                and self.selected_building_type.value == building_type
            ):
                color = COLORS["GREEN"]
            else:
                color = COLORS["BLUE"]
        ...
        for resource, amount in cost.items():
            ...
            cost_text = self.small_font.render(
                f"{resource}: {amount}", True, cost_color
            )
