...
from shared.constants.game_constants import COLORS, MAP_HEIGHT, MAP_WIDTH
...
class Minimap:
    ...
    def render(
        ...
        self._render_viewport_rectangle(
            screen, camera_x, camera_y, screen_width, screen_height
        )
        ...
        for y in range(MAP_HEIGHT):
            for x in range(MAP_WIDTH):
                ...
                mini_rect = pygame.Rect(
                    mini_x, mini_y, max(1, int(self.scale_x)), max(1, int(self.scale_y))
                )
                pygame.draw.rect(screen, color, mini_rect)

    def _render_fog_of_war(
        self, screen: pygame.Surface, game_state: GameState, player_id: str
    ):
            ...
            for x in range(len(shared_fog[y]) if y < len(shared_fog) else 0):
                if not shared_fog[y][x]:  # not explored
                    ...
                    fog_rect = pygame.Rect(
                        mini_x,
                        mini_y,
                        max(1, int(self.scale_x)),
                        max(1, int(self.scale_y)),
                    )
                    pygame.draw.rect(
                        screen, (50, 50, 50), fog_rect
                    )  # Dark gray for unexplored areas
        ...
        for building in game_state.buildings.values():
            ...
            pygame.draw.rect(
                ...
                (
                    mini_x,
                    mini_y,
                    max(2, int(building.size[0] * self.scale_x)),
                    max(2, int(building.size[1] * self.scale_y)),
                ),
            )
    ...
    def _render_viewport_rectangle(
        self,
        screen: pygame.Surface,
        camera_x: float,
        camera_y: float,
        ...
        if viewport_width > 0 and viewport_height > 0:
            viewport_rect = pygame.Rect(
                mini_left, mini_top, viewport_width, viewport_height
            )
            pygame.draw.rect(
                screen, COLORS["RED"], viewport_rect, 2
