...
class HealthBarRenderer:
    ...
    def draw_health_bar(
        ...
        entity: Union[Hero, Unit, Enemy, Building],
        camera_x: float,
        camera_y: float,
    ):
        ...
        if (
            bar_x + bar_width < -50
            or bar_x > screen.get_width() + 50
            or bar_y + self.bar_height < -50
            or bar_y > screen.get_height() + 50
        ):
            return

        # Background (dark red for missing health)
        pygame.draw.rect(
            screen, COLORS["DARK_GRAY"], (bar_x, bar_y, bar_width, self.bar_height)
        )
        ...
        if fill_width > 0:
            pygame.draw.rect(
                screen, health_color, (bar_x, bar_y, fill_width, self.bar_height)
            )

        # Border
        pygame.draw.rect(
            screen, COLORS["WHITE"], (bar_x, bar_y, bar_width, self.bar_height), 1
        )

    def draw_mana_bar(
        self, screen: pygame.Surface, hero: Hero, camera_x: float, camera_y: float
    ):
        ...
        if fill_width > 0:
            pygame.draw.rect(
                screen, COLORS["BLUE"], (bar_x, bar_y, fill_width, self.bar_height)
            )
